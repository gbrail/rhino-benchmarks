...
var HashMap = function () {
  ...
  Entry.prototype = {
    clone: function () {
      ...
      if (this._next) result._next = this._next.clone();
      return result;
    },
    toString: function () {
  ...
  AbstractMapIterator.prototype = {
    hasNext: function () {
      ...
      while (this._position < this._associatedMap._elementData.length) {
        if (!this._associatedMap._elementData[this._position]) this._position++;else return true;
      }
      return false;
    },
    _checkConcurrentMod: function () {
      if (this._expectedModCount != this._associatedMap._modCount) throw new Error("Concurrent HashMap modification detected");
    },
    _makeNext: function () {
  ...
  EntrySet.prototype = {
    ...
    remove: function (object) {
      ...
      if (!equals(entry._value, object.value)) return false;
  ...
  function HashMap(capacity, loadFactor) {
    if (capacity == null) capacity = DEFAULT_SIZE;
    if (loadFactor == null) loadFactor = 0.75;
    if (capacity < 0) throw new Error("Invalid argument to HashMap constructor: capacity is negative");
    if (loadFactor <= 0) throw new Error("Invalid argument to HashMap constructor: loadFactor is not positive");
  ...
  HashMap.prototype = {
    ...
    clear: function () {
      ...
      this._elementCount = 0;
      for (var i = this._elementData.length; i--;) this._elementData[i] = null;
      this._modCount++;
    },
    clone: function () {
    ...
    containsValue: function (value) {
      for (var i = this._elementData.length; i--;) {
        for (var entry = this._elementData[i]; entry; entry = entry._next) {
          if (equals(value, entry._value)) return true;
        }
      }
      return false;
    },
    ...
    _findKeyEntry: function (key, index, keyHash) {
      ...
      while (entry && (entry._origKeyHash != keyHash || !equals(key, entry._key))) entry = entry._next;
      return entry;
    },
    isEmpty: function () {
    ...
    _removeEntry: function (entry) {
      ...
      if (current == entry) this._elementData[index] = entry._next;else {
    ...
    _removeEntryForKey: function (key) {
      ...
      if (!entry) return null;
      if (!last) this._elementData[index] = entry._next;else last._next = entry._next;
      this._modCount++;
      this._elementCount--;
      return entry;
    },
...
function run() {
  var map = new HashMap();
  var COUNT = 90000;
  for (var i = 0; i < COUNT; ++i) map.put(i, 42);
  var result = 0;
  for (var j = 0; j < 5; ++j) {
    for (var i = 0; i < COUNT; ++i) result += map.get(i);
  }
  var keySum = 0;
  var valueSum = 0;
