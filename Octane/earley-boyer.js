...
function sc_withHandlerLambda(handler, body) {
  try {
    ...
    if (!e._internalException) return handler(e);else throw e;
  }
}
...
function sc_getprop(sym, key) {
  var ht = sc_properties[sym];
  if (ht) {
    if (key in ht) return ht[key];else return false;
  } else return false;
}
...
*/
function sc_minus(x) {
  if (arguments.length === 1) return -x;else {
    var res = x;
...
*/
function sc_div(x) {
  if (arguments.length === 1) return 1 / x;else {
    var res = x;
    for (var i = 1; i < arguments.length; i++) res /= arguments[i];
    return res;
  }
...
function sc_modulo(x, y) {
  ...
  if (remainder * y < 0) return remainder + y;else return remainder;
}
...
function sc_number2jsstring(x, radix) {
  if (radix) return x.toString(radix);else return x.toString();
}
function sc_jsstring2number(s, radix) {
  ...
  if (radix) {
    ...
    if (new RegExp("^[" + allowedChars + "]*$", "i").test(s)) return t;else return false;
  } else {
...
function sc_isList(o) {
  ...
  var turtle = o;
  while (true) {
    if (rabbit === null || rabbit instanceof sc_Pair && rabbit.cdr === null) return true; // end of list
...
sc_Char.prototype.sc_toWriteString = function () {
  ...
  if (entry) return entry;else return "#\\" + this.val;
};
...
function sc_makeJSStringOfLength(k, c) {
  var fill;
  if (c === undefined) fill = " ";else fill = c;
  var res = "";
  var len = 1;
...
function sc_copyVector(a, len) {
  if (len <= a.length) return a.slice(0, len);else {
    var tmp = a.concat();
    tmp.length = len;
    return tmp;
  }
...
function sc_filterMap(proc, l1, l2, l3) {
  if (l2 === undefined) return sc_filterMap1(proc, l1);else if (l3 === undefined) return sc_filterMap2(proc, l1, l2);
  // else
  var nbApplyArgs = arguments.length - 1;
  var applyArgs = new Array(nbApplyArgs);
  var revres = null;
...
*/
function sc_values() {
  if (arguments.length === 1) return arguments[0];else return new sc_Values(arguments);
}

/*** META ((export #t)) */
function sc_callWithValues(producer, consumer) {
  var produced = producer();
  if (produced instanceof sc_Values) return consumer.apply(null, produced.values);else return consumer(produced);
}
...
function sc_pregexpReplace(re, s1, s2) {
  ...
  if (re instanceof RegExp) {
    if (re.global) reg = re;else reg = new RegExp(re.source);
  } else {
...
function sc_pregexpReplaceAll(re, s1, s2) {
  ...
  var jss2 = sc_string2jsstring(s2);
  if (re instanceof RegExp) {
    if (re.global) reg = re;else reg = new RegExp(re.source, "g");
  } else {
...
function sc_hashtableGet(ht, key) {
  var hash = sc_hash(key);
  if (hash in ht) return ht[hash].val;else return false;
}
...
function sc_hashtableContains(ht, key) {
  var hash = sc_hash(key);
  if (hash in ht) return true;else return false;
}
var SC_HASH_COUNTER = 0;
function sc_hash(o) {
  if (o === null) return "null";else if (o === undefined) return "undefined";else if (o === true) return "true";else if (o === false) return "false";else if (typeof o === "number") return "num-" + o;else if (typeof o === "string") return "jsstr-" + o;else if (o.sc_getHash) return o.sc_getHash();else return sc_counterHash.call(o);
}
...
sc_Trampoline.prototype.restart = function () {
  ...
  while (true) {
    ...
    if (res instanceof sc_Trampoline) o = res;else return res;
  }
};
...
sc_Tokenizer.prototype.nextToken = function () {
  ...
  function readIdOrNumber(firstChar) {
    ...
    if (isNaN(res)) return new sc_Token(9 /*ID*/, res);else return new sc_Token(12 /*NUMBER*/, res - 0);
  }
  ;
  ...
  ;
  function readDot() {
    if (isWhitespace(port.peekChar())) return new sc_Token(10 /*DOT*/);else return readIdOrNumber(".");
  }
  ;
  function readSharp() {
    var c = port.readChar();
    ...
    if (c === "\\") {
      ...
      switch (tmp.length) {
        case 0:
          ...
          if (sc_isEOFObject(port.peekChar)) return new sc_Token(13 /*ERROR*/, "bad #-pattern2.");else return new sc_Token(20 /*CHAR*/, port.readChar());
        case 1:
          return new sc_Token(20 /*CHAR*/, tmp);
        default:
          var entry = sc_Char.readable2char[tmp.toLowerCase()];
          if (entry) return new sc_Token(20 /*CHAR*/, entry);else return new sc_Token(13 /*ERROR*/, "unknown character description: #\\" + tmp);
      }
    }
    ...
    while (true) {
      c = port.peekChar();
      if ((isWhitespaceOrEOF(c) || c === ')') && needing == "") return res;else if (isWhitespace(c) || needing == "") return new sc_Token(13 /*ERROR*/, "bad #-pattern4 " + c + " " + needing);else if (needing.charAt(0) == c) {
...
sc_Reader.prototype.read = function () {
  function readList(listBeginType) {
    function matchesPeer(open, close) {
      return open === 1 /*OPEN_PAR*/ && close === 2 /*CLOSE_PAR*/ || open === 3 /*OPEN_BRACE*/ && close === 4 /*CLOSE_BRACE*/ || open === 5 /*OPEN_BRACKET*/ && close === 6 /*CLOSE_BRACKET*/;
    }
    ;
    var res = null;
    while (true) {
      ...
      switch (token.type) {
        ...
        case 10 /*DOT*/:
          ...
          if (!matchesPeer(listBeginType, par.type)) throw "closing par doesn't match: " + listBeginType + " " + par.type;else return sc_reverseAppendBang(res, cdr);
        default:
  ...
  function readReference(nb) {
    if (nb in this.backref) return this.backref[nb];else throw "bad reference: " + nb;
  }
  ;
...
function sc_basename(p) {
  var i = p.lastIndexOf('/');
  if (i >= 0) return p.substring(i + 1, p.length);else return '';
}

/*** META ((export #t)) */
function sc_dirname(p) {
  var i = p.lastIndexOf('/');
  if (i >= 0) return p.substring(0, i);else return '';
}
...
function sc_toWriteString(o) {
  if (o === null) return "()";else if (o === true) return "#t";else if (o === false) return "#f";else if (o === undefined) return "#unspecified";else if (typeof o === 'function') return "#<procedure " + sc_hash(o) + ">";else if (o.sc_toWriteString) return o.sc_toWriteString();else return o.toString();
}
...
function sc_toDisplayString(o) {
  if (o === null) return "()";else if (o === true) return "#t";else if (o === false) return "#f";else if (o === undefined) return "#unspecified";else if (typeof o === 'function') return "#<procedure " + sc_hash(o) + ">";else if (o.sc_toDisplayString) return o.sc_toDisplayString();else return o.toString();
}
...
sc_Pair.prototype.sc_toWriteCircleString = function (symb, inList) {
  if (this[symb + "use"]) {
    ...
    if (inList) return '. #' + nb + '#';else return '#' + nb + '#';
  }
  if (this[symb]-- === 0) {
    // if we are the last use. remove all fields.
    delete this[symb];
  ...
  if (this[symb] !== undefined) {
    // implies > 0
    this[symb + "use"] = true;
    if (inList) res += '. #' + this[symb + "nb"] + '=';else res += '#' + this[symb + "nb"] + '=';
    inList = false;
  }
  if (!inList) res += "(";
...
String.prototype.sc_toDisplayString = function () {
  if (this.charAt(0) === sc_SYMBOL_PREFIX)
    ...
    return this.slice(1);else if (this.charAt(0) === sc_KEYWORD_PREFIX) return ":" + this.slice(1);else return this.toString();
};
String.prototype.sc_toWriteString = function () {
  if (this.charAt(0) === sc_SYMBOL_PREFIX)
    // TODO: care for symbols with spaces (escape-chars symbols).
    return this.slice(1);else if (this.charAt(0) === sc_KEYWORD_PREFIX) return ":" + this.slice(1);else return '"' + sc_escapeWriteString(this) + '"';
};
/* Exported Variables */
...
var sc_const_4_nboyer;
{
  ...
  BgL_nboyerzd2benchmarkzd2 = function () {
    ...
    return n = args === null ? 0 : args.car, BgL_setupzd2boyerzd2(), BgL_runzd2benchmarkzd2("nboyer" + sc_number2string(n), 1, function () {
      ...
      if (sc_isNumber(rewrites)) switch (n) {
        case 0:
  ...
  untranslate_term_nboyer = function (term) {
    ...
    var falseHead1130;
    var symbol_record;
    if (!(term instanceof sc_Pair)) return term;else {
  ...
  rewrite_nboyer = function (term) {
    ...
    var sc_lst_13;
    {
      ...
      if (!(term instanceof sc_Pair)) return term;else {
        ...
        while (true) {
          if (lst === null) return sc_term_12;else if ((term2 = lst.car.cdr.car, unify_subst_nboyer = null, one_way_unify1_nboyer(sc_term_12, term2)) !== false) return rewrite_nboyer(apply_subst_nboyer(unify_subst_nboyer, lst.car.cdr.cdr.car));else lst = lst.cdr;
        }
      }
    }
  };
  ...
  one_way_unify1_nboyer = function (term1, term2) {
    ...
    if (!(term2 instanceof sc_Pair)) {
      ...
      if (temp_temp !== false) return is_term_equal_nboyer(term1, temp_temp.cdr);else if (sc_isNumber(term2)) return sc_isEqual(term1, term2);else {
        unify_subst_nboyer = new sc_Pair(new sc_Pair(term2, term1), unify_subst_nboyer);
        return true;
      }
    } else if (!(term1 instanceof sc_Pair)) return false;else if (term1.car === term2.car) {
      lst1 = term1.cdr;
      lst2 = term2.cdr;
      while (true) {
        if (lst1 === null) return lst2 === null;else if (lst2 === null) return false;else if (one_way_unify1_nboyer(lst1.car, lst2.car) !== false) {
          lst1 = lst1.cdr;
          lst2 = lst2.cdr;
        } else return false;
      }
    ...
    if (x instanceof sc_Pair) {
      if (y instanceof sc_Pair) {
        if ((r1 = x.car, r2 = y.car, r1 === r2) !== false) {
          ...
          lst2 = y.cdr;
          while (true) {
            if (lst1 === null) return lst2 === null;else if (lst2 === null) return false;else if (is_term_equal_nboyer(lst1.car, lst2.car) !== false) {
              lst1 = lst1.cdr;
              lst2 = lst2.cdr;
            } else return false;
          }
  ...
  is_term_member_nboyer = function (x, lst) {
    ...
    var lst;
    while (true) {
      if (lst === null) return false;else if (is_term_equal_nboyer(x, lst.car) !== false) return true;else lst = lst.cdr;
    }
  };
  BgL_setupzd2boyerzd2 = function () {
    var symbol_record;
  ...
  BgL_testzd2boyerzd2 = function (n) {
    ...
    var sc_term_22;
    {
      ...
      sc_term_22 = term;
      if (!(sc_term_22 instanceof sc_Pair)) optrOpnd = sc_term_22;else optrOpnd = new sc_Pair(BgL_sc_symbolzd2ze3symbolzd2record_1ze3_nboyer(sc_term_22.car), translate_args_nboyer(sc_term_22.cdr));
      ...
      sc_display(" rewrites");
      sc_newline();
      if (answer !== false) return rewrite_count_nboyer;else return false;
    }
  };
}
/* Exported Variables */
    ...
    var ind;
    {
      ind = function (nt, sc_nts_10) {
        var i;
        {
          ...
          while (true) {
            if (i >= 0) {
              if (sc_isEqual(sc_nts_10[i], nt)) return i;else --i;
            } else return false;
          }
        }
      };
        ...
        var BgL_sc_nbzd2derivzd2treesza2_48za2;
        {
          sc_ind_43 = function (nt, sc_nts_49) {
            var i;
            {
              ...
              while (true) {
                if (i >= 0) {
                  if (sc_isEqual(sc_nts_49[i], nt)) return i;else --i;
                } else return false;
              }
            }
          };
            ...
            var BgL_sc_ntza2_90za2;
            {
              ...
              if (BgL_sc_ntza2_90za2 !== false) {
                ...
                while (true) {
                  if (l instanceof sc_Pair) {
                    sc_conf_89 = l.car;
                    if ((state = sc_states_88[j], conf_set = state[sc_conf_89 + 1], conf_set !== false ? conf_set[i + 5] : false) !== false) return true;else l = l.cdr;
                  } else return false;
                }
              } else return false;
            }
                            ...
                            while (true) {
                              if (l instanceof sc_Pair) {
                                ...
                                if (ender_set !== false) {
                                  ...
                                  while (k >= 0) {
                                    {
                                      if (k >= i) if (BgL_sc_confzd2setzd2adjoinza2za2_46z00(sc_states_129, BgL_sc_statesza2_128za2, k, prev, i) !== false) BgL_sc_confzd2setzd2adjoinza2za2_46z00(sc_states_129, BgL_sc_statesza2_128za2, j, ender, k);
                                      k = ender_set[k + 5];
                                    }
                                  }
